(function ($) {
  ...
  // Optional: Allow user to manually skip loader
  $(".preloader-close").on("click", function () {
	  ...
	  var bannerSlider2 = new Swiper(".banner-slider-2", {
		...
		on: {
		  slideChangeTransitionStart: function () {
			...
			document.querySelectorAll(".bnox_76").forEach((banner) => {
			  banner.classList.remove("zoom-in-slow");
			  banner.classList.add("zoom-reset");
			});
  
			// Add animation to the active slide
			const activeSlide = this.slides[this.activeIndex];
			const banner = activeSlide.querySelector(".bnox_76");
			if (banner) {
			  banner.classList.remove("zoom-reset"); // Remove reset class
			  banner.classList.add("zoom-in-slow"); // Add zoom animation
			}
		  },
		},
	  });
